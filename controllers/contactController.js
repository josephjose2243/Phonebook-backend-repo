...
const createContact = async (req, res) => {
  try {
    ...
    const phone = phoneNumbers[0].number; // Save first phone number as "phone"
    
    // Saving both `phone` and `phones` (array of phone numbers)
    const newContact = new Contact({
      name,
      phone,  // Flat phone field
      phones: phoneNumbers,  // Array of phones
      email: email || '', 
      company: company || '', 
    });
